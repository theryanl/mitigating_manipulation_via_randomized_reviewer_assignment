...
""" This file runs multiple tests of the similarity matrix LP + bvn process on a user-specified file. It also computes the result formed by the AAAI method. Then, it outputs data that allows the user to compare the two. """
...
if (obj_type == 0): #TPMS objective
    for Q in range(10, 110, 10): #loops through Q values (multiplied by 100)
        ...
        try:
            ...
            TPMS_score = -1
            #can uncomment the following line for additional clarity
            #print("infeasible")
        ...
        for i in range(num_trials):
            AAAI_result = os.popen(f"python3 ./AAAI_TPMS.py {my_dataset} {Q/100} {k} {l}") #running the LP portion
    ...
    for Q in range(10, 110, 10): #looping through values for Q (multiplied by 100)
        ...
        Q_result = os.popen(f"python3 ./LP_max_min_fairness.py {my_dataset} {Q} {k} {l}") #running the LP portion
        lines = Q_result.readlines()
        ...
        if max_min_fairness != -1: # if feasible
            ...
            #can uncomment out the following to average over bvn trials
            #currently this gets the result of the fractional LP.
            '''
            scores = []
            for i in range(num_trials):
                os.system("./a.out < output.txt > output_bvn.txt")
            ...
            Q_results.append(s)
            #Q_stderrs.append(s)
            #no stderr for now, since randomness is only in bvn portion

        #can uncomment out to compare with the AAAI max-min fairness version's results
        '''
        #getting results for AAAI
        scores = []
        count_infeasible = 0
...
np.save(name, [Q_values, Q_results, AAAI_results, Q_stderrs, AAAI_stderrs]) #saves as npy file
